...
class Credibility:
    ...
    def get_credibility(self, df, text_col):
        if self.method == 'gunning-fog':
            df['crebility_index'] = df.text_col.map_partitions(self._gunning_fog)
        elif self.method == 'flesch':
            df['crebility_index'] = df.text_col.map_partitions(self._flesch_reading_ease)
        elif self.method == 'smog':
            df['crebility_index'] = df.text_col.map_partitions(self._smog_index)
        elif self.method == 'dale-chall':
            df['crebility_index'] = df.text_col.map_partitions(self._dale_chall_readability_score)
        else:
